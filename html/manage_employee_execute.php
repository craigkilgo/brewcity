...
if (isset($_POST['submitted'])) {
	...
	{ // If everything's OK.
		...
		{
		$q = "DELETE FROM employee WHERE user_id ='" . $user_id  . "'";		
		$r = @mysqli_query ($dbc, $q); // Run the query.
		
			//logging
				$user = $_SESSION['first_name'] . ' ' . $_SESSION['last_name'];
				$action = 'Employee deleted ' . $email . ' ' . $fn . ' ' . $ln;
    ...
    <strong>Success!</strong> Customer deleted:<br />');
		...
		{
		$q = "UPDATE employee SET first_name = '" . $fn . "', last_name ='" . $ln . "', email ='" . $email  ."', phone ='" . $phone . "', role='" . $role . "' WHERE user_id ='" . $user_id  . "'";		
		$r = @mysqli_query ($dbc, $q); // Run the query.
				
				//logging
				$user = $_SESSION['first_name'] . ' ' . $_SESSION['last_name'];
				$action = 'Employee modified ' . $email . ' ' . $fn . ' ' . $ln;
				$qlog = "INSERT INTO logs (user_id, action, action_table, action_time) VALUES ('$user', '$action', 'customers', NOW())";		
				$rlog = @mysqli_query ($dbc, $qlog); // Run the query.
...
?>
			...
			<div class="col-lg-8 col-md-8 col-sm-10 col-xs-12">
                            ...
                            Home</button> </a> <a href="manage_employees.php"><button class="btn btn-skin" id="manage">
                            Manage Employees</button></a><br />
</div>
